...
// 1. 컴포넌트 초기화 될 때 json-server로 연결해서 전체 목록 가지고 와야함
// 2. 백엔드 서버로부터 가지고 온 목록을 rodolist에 넣어줌
// 3. 화면에 todolist목록을 보일 것
  ...
  try {
   ...
   //2. 키워드 추가후
    ...
    if (keyword.trim()) {
      ...
      //npm list -g --depth=0  버전확인
      params['todo_like'] = keyword.trim()
    }
    ...
    console.error("예상치 못한 에러가 발생");
  }
};

fetchTodoList(); // 초기 데이터 로드

fetchTodoList(); //시작하자마자 함수를 호출하고 싶을 때 써줌
...
const totalPages =
  computed(() => Math.ceil(states.totalcount / states.perPage)) || 1;
